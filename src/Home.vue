...
const fetchType = types => {
  try {
    const jpType = types.map(item => {
      try {
        const type = typeDetails.value.find(t => {
          return t.name === item.type.name
        });
        const name = getTypeJapaneseName(type.details.names);
        return name;
      } catch (error) {
...
const drawGacha = async (gachaCount = 1) => {
  ...
  try {
    const pokemonsPromises = Array.from({ length: gachaCount }, async () => {
      return await drawSinglePokemon(allPokemon.value);
    });
...
const drawSinglePokemon = async (pokemonData) => {
  ...
  const pokemonDetails = await fetchFromApi(selectedPokemon.url);
  ...
  const flavor_text = await fetchFlavorText(pokemonDetails.species);
  const types = await fetchType(pokemonDetails.types);
  return {
...
<template>
  ...
  <div class="px-3 pt-6 text-center flex justify-center flex-col items-center">
  ...
  <!-- 取得したポケモンを表示 -->
  <div v-if="pokemons.length >= 1" class="p-3 max-w-7xl mx-auto">
    <ul class="mb-24 grid grid-cols-12 md:grid-cols-10 gap-8 justify-center items-center">
      ...
      >
        <div @click="showPokemonDetails = pokemon.id">
        ...
        <!-- ポケモン詳細表示モーダル -->
        <div v-if="showPokemonDetails && showPokemonDetails == pokemon.id" class="fixed inset-0 z-50 bg-black/30 w-full">
                ...
                <div class="relative bg-white rounded-lg shadow pb-4">
                  ...
                  <div class="px-4">
                    <div class="flex justify-center">
                      <img :src="pokemon.img" alt="" class="w-48">
                    </div>
                    <ul>
                      <li v-for="type in pokemon.types" :key="type" class="m-1 text-xs font-medium bg-gray-200 rounded inline-block py-1 px-2">
                        {{ type }}
                      </li>
                    </ul>
                    <h3 class="font-bold p-2">{{ pokemon.name }}</h3>
