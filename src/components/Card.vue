...
<template>
  <div 
    class="relative col-span-6 rounded md:col-span-2 w-40 cursor-pointer flex justify-center items-center mx-auto flex-col h-52"
    :class="[
      ringColor, 
      'transition-all duration-500 ease-in-out',
      isFlipped ? 'transition-all scale-x-100 ring-2 shadow-xl' : '-scale-x-100 bg-gray-700 border-2 shadow'
    ]"
    @click="isFlipped ? showPokemonDetails = pokemon.id : isFlipped = true" 
  >
    <div v-if="!isFlipped" class="absolute inset-0 flex justify-center items-center -scale-x-100 animate-pulse text-white font-bold">
      Tap to open
    </div>

    <div 
      :class="[isFlipped ? 'visible opacity-100' : 'invisible opacity-0']" 
      class="z-10 w-full p-4 mx-auto flex justify-center flex-col items-center transition-opacity duration-300 ease-in-out">
