...
const drawGacha = async (gachaCount = 1) => {
  if (!pokemonsData.value) return;

  isLoadingGacha.value = true;
  pokemons.value = [];
  
  for (let i = 0; i < gachaCount; i++) {
    const pokemonIndex = Math.floor(Math.random() * pokemonsData.value.count);
    const selectedPokemon = pokemonsData.value.results[pokemonIndex];
    const detailsResponse = await axios.get(selectedPokemon.url);
...
const {isLoading, isError, data: pokemonsData, error } = useQuery({
  queryKey: ['pokemons'],
...
<template>
  ...
  <div v-if="isLoading || isLoadingGacha" class="z-50 fixed inset-0 flex flex-col justify-center items-center">
    <div class="animate-spin h-10 w-10 border-4 border-blue-500 rounded-full border-t-transparent"></div>
    <div class="font-medium pt-1 text-gray-800 animate-pulse">ガチャロード中</div>
  </div>

  <div :class="[isLoading || isLoadingGacha || pokemons.length >= 1 ? 'pt-6' : 'pt-36']" class="px-3 text-center flex justify-center flex-col items-center">
    <h3 class="font-medium text-md sm:text-lg md:text-xl text-gray-800">
      ポケモンガチャで運試しに挑戦！何が出るかな？
    </h3>
